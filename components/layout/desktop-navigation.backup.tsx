...
import { EXAM_CATEGORIES, NAVIGATION_ITEMS } from "@/lib/constants";
...
> = {
  banking: [
    ...
    {
      id: "rbi",
      label: "RBI & Regulatory Exams",
    },
    {
      id: "insurance",
      label: "Insurance Exams",
    },
  ],
...
function ExamLogo({
  ...
  return (
    <span
      aria-hidden="true"
      className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full border border-slate-200 bg-slate-50 px-1 text-xs font-black text-[#E13032] transition duration-200 group-hover:border-red-100 group-hover:bg-red-50"
    >
      {value}
    </span>
  );
  ...
  return (
    ...
    >
      ...
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        <div className="flex h-11 items-center gap-1 overflow-x-auto scrollbar-none">
          <Link
            ...
            className="inline-flex h-8 shrink-0 items-center rounded-lg bg-red-50 px-3 text-xs font-extrabold text-[#E13032] transition hover:bg-red-100"
          >
            All Exams
          </Link>
          ...
          {EXAM_CATEGORIES.map((category) => {
            ...
            return (
              <Link
                ...
                onClick={handleMegaMenuClose}
                className={`group inline-flex h-8 shrink-0 items-center gap-1.5 rounded-lg px-3 text-xs font-semibold transition ${
                  isActive
          ...
          {NAVIGATION_ITEMS.slice(1).map(
            (item) => (
              <Link
                key={item.href}
                href={item.href}
                onClick={handleMegaMenuClose}
                className="inline-flex h-8 shrink-0 items-center gap-1 rounded-lg px-3 text-xs font-semibold text-slate-600 transition hover:bg-slate-50 hover:text-[#E13032]"
              >
                {item.label}
              </Link>
            ),
          ...
          <Link
            href="/free-resources"
            onClick={handleMegaMenuClose}
            className="inline-flex h-8 shrink-0 items-center gap-1.5 rounded-lg px-3 text-xs font-bold text-[#E13032] transition hover:bg-red-50"
          >
            Free Resources
      ...
      <div
        ...
        onMouseEnter={handleMegaMenuHoverEnter}
        onMouseLeave={handleMegaMenuHoverLeave}
        className={`absolute left-1/2 top-1 z-[80] w-[min(1180px,calc(100vw-32px))] -translate-x-1/2 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-[0_24px_70px_rgba(15,23,42,0.18)] transition-all duration-200 ease-out ${
          activeMegaMenu
      ...
      >
        {/* Page overlay */}
        <div
          className={`fixed inset-x-0 top-0 -z-10 h-screen bg-slate-950/25 backdrop-blur-[1px] transition-opacity duration-200 ${
            activeMegaMenu
              ? "opacity-100"
        ...
        <div className="grid h-[535px] grid-cols-[250px_310px_minmax(0,1fr)]">
          {/* Left */}
          <div className="overflow-y-auto border-r border-slate-200 bg-slate-50/60 p-5 scrollbar-thin">
            <div className="mb-5 border-b border-slate-200 px-2 pb-4">
              <p className="text-[11px] font-black uppercase tracking-[0.15em] text-[#E13032]">
                {getMenuTitle(
                ...
                (category) => {
                  ...
                  return (
                    <button
                      ...
                      className={`group flex w-full items-center gap-3 rounded-xl border px-3 py-3.5 text-left transition-all duration-150 ${
                        selected
                          ? "border-red-100 bg-white text-[#E13032] shadow-sm"
                      ...
                      <span
                        className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-lg transition ${
                          selected
                            ...
                            : "bg-slate-50 text-slate-400 group-hover:bg-red-50 group-hover:text-[#E13032]"
                        }`}
                      >
                        <Icon
                          className="h-4 w-4"
          ...
          {/* Middle */}
          <div className="overflow-y-auto border-r border-slate-200 bg-white p-6 scrollbar-thin">
            <div className="mb-6">
              <p className="text-[11px] font-black uppercase tracking-[0.15em] text-slate-400">
                Explore
              </p>

              <h3 className="mt-1 text-[19px] font-black tracking-tight text-slate-950">
                {
                ...
                (subcategory) => {
                  ...
                  return (
                    <button
                      ...
                      className={`group flex w-full items-center justify-between rounded-xl border px-3 py-3.5 text-left transition-all duration-150 ${
                        selected
            ...
            <Link
              ...
              onClick={handleMegaMenuClose}
              className="mt-6 flex items-center gap-2 rounded-xl bg-slate-50 px-4 py-3 text-xs font-bold text-slate-700 transition hover:bg-red-50 hover:text-[#E13032]"
            >
              View all exams
          ...
          {/* Right */}
          <div className="min-w-0 overflow-y-auto bg-white p-7 scrollbar-thin">
            <div className="mb-5 flex items-end justify-between gap-4">
              ...
              <Link
                ...
                onClick={handleMegaMenuClose}
                className="hidden items-center gap-1 text-xs font-bold text-[#E13032] sm:inline-flex"
              >
                View all
            ...
            <div className="grid grid-cols-2 gap-3 lg:grid-cols-3 xl:grid-cols-4">
              {currentExams.map((exam) => (
                <Link
                  ...
                  onClick={handleMegaMenuClose}
                  className="group flex min-h-[112px] flex-col items-center justify-center rounded-2xl border border-slate-200 bg-white p-4 text-center shadow-[0_1px_2px_rgba(15,23,42,0.03)] transition-all duration-200 hover:-translate-y-0.5 hover:border-red-200 hover:bg-red-50/50 hover:shadow-[0_10px_24px_rgba(15,23,42,0.08)]"
                >
                  <ExamLogo
                    value={exam.logo}
                  />

                  <span className="mt-3 line-clamp-2 text-[12px] font-bold leading-5 text-slate-700 transition-colors group-hover:text-slate-950">
                    {exam.name}
                  </span>
                </Link>
              ))}
