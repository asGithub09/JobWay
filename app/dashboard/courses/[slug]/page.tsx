...
import {
  ...
  getStudentCourseCheckpoint,
  submitStudentCourseCheckpoint,
  type Course,
...
type CourseLearningItem = CourseLesson & {
  id?: string;
  _id?: string;
  type?: string;
};
type CourseModule = {
  ...
  learningItems?: CourseLearningItem[];
};
...
type StudentCourseCheckpointQuestion = {
  id?: string;
  _id?: string;
  question: string;
  options: string[];
...
type StudentCourseCheckpointResult = {
...
type StudentCourseCheckpointAttempt = {
  _id?: string;
  status?: string;
  answers?: Array<{
    question: string;
...
type SubmitStudentCourseCheckpointResponse = {
  success?: boolean;
  message?: string;
...
function getModuleLessons(module: CourseModule): CourseLesson[] {
  if (module.learningItems?.length) {
    return module.learningItems.map(normalizeLearningItem);
  }
...
export default function ProtectedCourseLearningPage() {
  ...
  function isModuleCompleted(
    ...
    const lessons =
      getModuleLessons(modules[moduleIndex] || {});
          ...
          <div className="grid gap-6 lg:grid-cols-[320px_minmax(0,1fr)] xl:grid-cols-[340px_minmax(0,1fr)]">
            ...
            <aside className="hidden lg:block">
              <div className="sticky top-[145px] max-h-[calc(100vh-165px)] overflow-y-auto rounded-3xl border border-slate-200 bg-white p-4 shadow-sm">
                <div className="mb-4 flex items-center justify-between gap-3 border-b border-slate-100 px-2 pb-4">
                  <div>
                    <p className="text-xs font-black uppercase tracking-wider text-[#E13032]">
                      Course Contents
                    </p>
                  ...
                  <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl border border-red-100 bg-red-50 text-[#E13032]">
